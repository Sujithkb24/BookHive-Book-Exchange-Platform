// Full updated code for MyOrdersPage with Cancel Order button
...
const MyOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3000/api/order/myorders', {
        method: 'GET',
        headers: {
  ...
  const handleOrderReceived = async (orderId, orderIndex) => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/order/updaterecieved', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
      ...
      orders[orderIndex].isRecieved = true;
      setOrders([...orders]);
    } catch (err) {
  ...
  const handleCancelOrder = async (orderId, orderIndex) => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/order/ordercancelbybuyer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
              ...
              <div key={index} className="bg-white shadow-lg rounded-xl overflow-hidden p-6">
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    disabled={order.isRecieved}
                    className="bg-green-600 text-white py-2 px-4 rounded hover:bg-green-700 disabled:opacity-50"
                  >
                    {order.isRecieved ? 'Order Received' : 'Mark as Received'}
                  </button>

                  <button
                    onClick={() => handleCancelOrder(order.orderId, index)}
                    disabled={actionLoading[`cancel-${index}`] || order.isRecieved}
